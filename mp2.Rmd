...
cleancontributions <- contributions %>%
  select(cmte_id, name, city, state, zip_code, transaction_amt, cand_id, tran_id) 
...
committeecontributionlist <- candcontjoin %>%
  full_join( committeename, by = "cmte_id")
```
...
committeebypartyfinal <-committeebyparty %>%
  full_join( committeegroup, by = "cmte_name") %>%
  mutate( percentdem = (countofdem / countofcontributions) * 100) %>%
  mutate( pecentrep = (countofrep / countofcontributions) * 100) 
```
